...
import datetime,re,time,os,logging,mailbox,glob
...
def process_emls(path,progress,loading,progress_bar,_error):
    ...
    emails = []
    if len(emls)>0:
        loading.emit((2,f"Total Emls files detected : {len(emls)}",len(emls)))
        ...
        if conn is not None:
            with conn:
                ...
                if row is None:
                    ...
                    value = (current_time,len(emls),int(row[4])+1,int(row[4])+1+len(emls))
                    cur.execute(insert_query,value)
        ...
        with open(eml, 'rb') as fp:
            ...
            if  "support@idrive.com" in regex.findall(eml_msg['From']) or \
                ...
                try:
                    ...
                    error.exception(f"Logging Error: {e} - {eml_msg['X-Original-Sender']} - {eml_msg['Subject']}")
        ...
        if conn is not None:
            with conn:
                ...
                if row is None:
                    ...
                    value = (current_time,total_emails,int(row[4])+1,int(row[4])+1+total_emails)
                    cur.execute(insert_query,value)
                    ...
                    if  "support@idrive.com" in regex.findall(msg['From']) or \
                        ...
                        for content in email_data.read_email_payload():
                                try:
                                    ...
                                    error.exception(f"ParseMBox: {e} : {msg['X-Original-Sender']} - {msg['Subject']}")
                        try:
                            ...
                            from_email.append([items,emails[:3],msg['Subject'],local_time,attachments,email_data])
                            progress.emit([items,emails[:3],msg['Subject'],local_time,attachments,email_data])
                        ...
                        try:
                            ...
                            progress.emit([items,regex.findall(msg['X-Original-Sender']),msg['Subject'],local_time,attachments,body_content])
                            from_email.append([items,regex.findall(msg['X-Original-Sender']),msg['Subject'],local_time,attachments,body_content])
                            cur.execute(query,value)
                        except Exception as e:
                        ...
                        try:
                            ...
                            body_content = mbox.GmailMboxMessage(msg)
                            progress.emit([items,regex.findall(msg['From']),msg['Subject'],local_time,attachments,body_content])
                            from_email.append([items,regex.findall(msg['From']),msg['Subject'],local_time,attachments,body_content])
                            cur.execute(query,value)
                        except Exception as e:
