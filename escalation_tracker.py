...
agent_csv_file_path = os.path.join(os.path.expanduser("~"), "Desktop", "agents.csv")
...
note_edit_body = {"title": "", "body": "", "__CSRFToken__": "", "commit": "save"}
...
columns = [
    ...
    "Developer Update",
    "Response to Dev",
    "Response Pending to Dev Update",
    "Notes",
    "Events",
]
    ...
    for tr in soup.select(".list tbody tr"):
        for td in tr.select("td:nth-child(2)"):
            if td.get_text().strip() == ticket:
                url = "https://ticket.idrive.com" + td.find_all("a")[0]["href"]
                break
    return url
...
def find_responses(soup, selector1, selector2):
    ...
    for b, time in zip_longest(soup.select(selector1), soup.select(selector2)):
        responses.append(
            {
                "agent": b.get_text().strip(),
                "time": time.get_text().strip(),
                "timestamp": datetime.strptime(
                    time.get_text().strip().replace("\u202f", " "), "%m/%d/%y %I:%M %p"
                ).timestamp(),
            }
        )
    return responses
...
def find_notes_with_body(soup, selector1, selector2):
    ...
    # .thread-entry.response .header     --> agent response
    notes = []
    for b, time, body in zip_longest(
        soup.select(selector1), soup.select(selector2), soup.select(note_body)
    ):
        notes.append(
            {
                "agent": b.get_text().strip(),
...
def find_due_date(
    session, departments, escalated, notes, ticket, visited, assigned_agent, responses
):
    i = 0
    dev = None
    diff = None
    ...
    while i < notes_len:
        ...
        if (
            ...
            and note["timestamp"] > escalated["timestamp"]
            and note["agent"] in visited[escalated["department"]]
        ):
            ...
            while j < responses_len:
                ...
                if res["timestamp"] > note["timestamp"]:
                    if (
                        res["agent"] == assigned_agent
                        or res["agent"] in visited["India Support"]
                    ):
                        agent_res = res
                else:
                    break
    ...
    return {
        "ticket": ticket,
        "due": str(round(diff.total_seconds() / 86400))
        if diff is not None
        else str(None),
        "escalated": escalated,
        ...
        "agentUpdatePending": str(agent_update_pending)
...
def get_ticket_html_content(session, ticket):
    try:
        ...
        assigned_department = (
            soup.select(depart_name_selector)[0]
            .get_text()
            .split("Support /")[-1]
            .strip()
        )
        assigned_agent = (
            soup.select(assigned_agent_selector)[0]
            .get_text()
            .split("Support /")[-1]
            .strip()
        )
...
def find_res_pending(responses, messages):
    ...
    if messages:
        ...
        while i < res_len:
            ...
            i += 1
        return (
            str(datetime.now() - time(no_response["timestamp"]))
            if no_response
            else "Updated"
        )
    return "Updated"
...
def escalation_worker(_dict):
    ...
    processed = []
    idx = 0
    _dict["agents_in_department"] = extract_departments("./logs/departments.csv")
    visited = {}
    session = _dict["session"]
    _dict["departments"] = set(get_agents_or_departments(session, departmentURL))
    with ThreadPoolExecutor(n_workers) as exe:
        futures = [
            exe.submit(process_ticket, ticket, visited, _dict)
        ...
        for idx in range(5):
            if messages:
                if len(messages) > 0 and len(responses) > 0:
                    ...
                    time_diff_dict["Thread_" + str(idx + 1)] = (
                        "Received: "
                        + msg["time"]
                        + " || "
                        + "Replied: "
...
def process_agent_ticket(ticket, visited, payload):
    ...
    if html_soup is not None:
        ...
        if isinstance(soup, bs4.BeautifulSoup):
            ...
            messages = find_messages(soup, selector6, selector7)
            deepcopy_res = deepcopy(responses)
            res_mapped = msg_res_eta(ticket, deepcopy_res, messages)
            per = math.ceil((idx / payload["total"]) * 100)
            payload["pBar"].emit(per)
            return res_mapped
        else:
...
def agent_tracker_worker(_dict):
    ...
    with ThreadPoolExecutor(n_workers) as exe:
        futures = [
            exe.submit(process_agent_ticket, ticket, visited, _dict)
            for ticket in _dict["tickets"]
        ]
        for future in futures:
            due = future.result()
...
def process_notes(ticket, payload):
    ...
    if html_soup is not None:
        ...
        if isinstance(soup, bs4.BeautifulSoup):
            ...
            if mode["note"] == "add":
                add_note(
                    session, "Need Update", ticket, url, CSRFToken, ticket_id, error
                )
            elif mode["note"] == "edit":
                note_id = mode["id"]
                edit_url = note_edit_url.format(ticket_id, note_id)
                edit_note(session, "Need Update", ticket, edit_url, CSRFToken, error)
            per = math.ceil((idx / payload["total"]) * 100)
            payload["pBar"].emit(per)
...
def note_update_worker(_dict):
    ...
    with ThreadPoolExecutor(3) as exe:
        futures = [exe.submit(process_notes, ticket, payload) for ticket in tickets]
        for future in futures:
            done = future.result()
        ...
        if row["computed"]:
            df.append(
                [
                    ...
                    row["assigned"],
                    None
                    if row["updatedAgent"] is None
                    else row["updatedAgent"]["agent"],
                    None
                    if row["updatedAgent"] is None
                    else datetime.now() - time(row["updatedAgent"]["timestamp"]),
                    None
                    if row["updatedAgent"] is None
                    else row["updatedAgent"]["body"],
