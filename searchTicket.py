...
from login import ticket_session

idx = 0
email_idx = 0
expiredAt = {"idx": 0, "email_idx": 0, "subject_idx": 0, "err_idx": 0}
subject_idx = 0
err_idx = 0
n_workers = 5

ticket_reply_reg = re.compile(
...
idriveinc_domain_regex = re.compile(
    r'''([a-zA-Z0-9.!#$%&'*+-/=?^_`{|}~-]+@[a-zA-Z0-9.!#$%&'*+-/=?^_`{|}~-]+(?:\.idrive\.com|\.remotepc\.com|\.ibackup\.com|\.idriveinc\.com|\.idrivecompute\.com|\.idrivemirror\.com))''', re.MULTILINE)

idrivedomain_regex = re.compile(
...
searchURL_sorted_new = 'https://ticket.idrive.com/scp/tickets.php?a=search&search-type=&query={}&dir=1&sort=2'
...
def fetchExcludeList():
    ...
    try:
        ...
        error.exception(
            f'File Error: Exclude file not found. Using Default - {e}')
        return None


def ticketing(_dict):
    global n_workers
    global subject_idx, email_idx, idx, err_idx, expiredAt
    global exclude_list

    expiredAt = _dict["startFrom"]
    idx = expiredAt["idx"]
    email_idx = expiredAt["email_idx"]
    err_idx = expiredAt["err_idx"]
    subject_idx = expiredAt["subject_idx"]
    ...
    with ThreadPoolExecutor(n_workers) as exe:
        futures = [exe.submit(fetch_ticket, [mail], _dict, event)
                   for mail in _dict["emails"][idx:]]
    ...
    if conn is not None:
        with conn:
            ...
            if email_cur is not None:
                val = (int(email_cur[0])-email_idx,
                       int(email_cur[0]), int(mbox_cur[0]))
                cur.execute(
                    '''UPDATE mbox SET EMAILS_START=?, EMAILS_END=? WHERE ID=?''', val)
            if subject_cur is not None:
                val = (int(subject_cur[0])-subject_idx,
                       int(subject_cur[0]), int(mbox_cur[0]))
                cur.execute(
                    '''UPDATE mbox SET SUBJECT_START=?, SUBJECT_END=? WHERE ID=?''', val)
            cur.connection.commit()
    _dict['warning'].emit(abnormal)


def fetch_ticket(chunk_tickets, _dict, event):
    global idx, subject_idx, email_idx, exclude_list, err_idx, expiredAt
    if event.is_set():
        idx = idx+1
            ...
            if ticket is not None:
                for item in ticket[1]:
                    if len(item) > 0:
                        ...
                        if item.lower() not in exclude_list and len(re.findall(idrivedomain_regex, item.lower())) == 0:
                            r = search_query(
                                _dict['session'], event, _dict, searchQuery=item)
                            if isinstance(r, str):
                                ...
                                try:
                                    ...
                                    # search = decoded_content.__contains__(
                                    ...
                                    if len(status) == 0:
                                        ...
                                        try:
                                            ...
                                            if sub_obj == True:
                                                ...
                                                if len(re.findall(idriveinc_domain_regex, val)) == 0:
                                                    matchProbability = extractCustomerMessages(
                                                        _dict['session'], status, ticket[5])
                                                    # if not extractCustomerMessages(_dict['session'],status,ticket[5]):
                                                    missing_ticket.debug(val)
                                                    value = (
                                                        item, ticket[2], ticket[3], f'''{ticket[4]}''', matchProbability)
                                                    cur.execute(
                                                        sub_query, value)
                                                    cur.connection.commit()
                                                    temp = ticket
                                                    ...
                                                    temp.insert(
                                                        2, matchProbability)
                                                    _dict['subjectNotFound'].emit(
                                                        temp)

                                            elif isinstance(sub_obj, list):
                                                for tick in sub_obj:
                                                    r = search_query(
                                                        _dict['session'], event, _dict, searchQuery=tick)
                                                    ticket_soup = BeautifulSoup(
                                                        r.content, 'html5lib')
                                                    ticket_found = extract_search(
                                                        ticket_soup)
                                            ...
                                            error.exception(
                                                f"Error-Level 2: {e} : {ticket}")

                                except Exception as e:
                                    error.exception(
                                        "Error-Level 1: {} : {}".format(e, ticket))

                            elif str(r.status_code).startswith('4'):
                                error.exception(
                                    f"Error-Unauthorized: {ticket}")
                                err_idx += 1
                                temp = ticket
                                temp[0] = err_idx
                                _dict['error'].emit(
                                    ('403-Unauthorized', temp, item))
                                event.set()
                                ...
                                temp[0] = err_idx
                                _dict['error'].emit(
                                    ('500-Ticket Error', temp, item))

                            elif str(r.status_code).startswith('3'):
                                # resource not found - logged out
                                error.exception(
                                    f"Error-InvalidSession: {ticket}")
                                err_idx += 1
                                temp = ticket
                                temp[0] = err_idx
                                _dict['error'].emit(
                                    ('Invalid Session', temp, item))
                                event.set()
...
def auto_login(session):
    ...
    if conn is not None:
        ...
        if row is not None:
            ticket_session(row[0], row[1], session, isExpired=True)
        conn.close()


def search_query(s, event=None, _dict=None, ticketURL=None, searchQuery=None):
    global idx, subject_idx, email_idx, err_idx, expiredAt
    try:
        if searchQuery is not None:
            url = searchURL_sorted_new.format(
                urllib.parse.quote_plus(searchQuery.strip()))
        else:
            url = ticketURL
        ...
        if loginMsg is not None and len(loginMsg) > 0:
            ...
            if loginMsg == "Invalid login" or loginMsg == "Access denied" or loginMsg == "Authentication Required":
                ...
                _dict["expiredAt"].emit(expiredAt)
                event.set()
        return r
        # r  = s.get(url,headers=headers)
        # return r
...
def extract_search(soup):
    list_obj = {}
    ticketHomeurl = "https://ticket.idrive.com"
    for count, tbody in enumerate(soup.find_all('tbody')):
        for count, tr in enumerate(tbody.find_all('tr')):
            search_obj = {}
            for i, td in enumerate(tr.find_all('td')):
                if i == 1:
                    ...
                    children = td.findChildren(
                        'span', attrs={"class": "truncate"}, recursive=True)
...
def compare_tickets(results, email_subject):
    for item in results.values():
        ...
        if isinstance(parsed_email_subject, list):
            ...
            return None
    missing_sub_logger.warning("Email_Subject = {} : Parsed Subject = {}".format(
