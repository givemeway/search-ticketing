    ...
    for count, tr in enumerate(soup.find_all('tr')):
        ...
        for i, td in enumerate(tr.find_all('td')):
            if i == 1:
                ...
                attachments = td.findChildren(
                    'i', attrs={"class": "icon-paperclip"}, recursive=True)
...
def search_ticket(s, string, result):
    r = search_query(s, searchQuery=string)
    soup = BeautifulSoup(r.content, 'html5lib')
    data = extract_search(soup)
    idx = 1
    if len(data) > 0:
        for item in data.values():
            result.emit([idx, item['ticket'], item['subject'], item['status'],
                        item['created'], item['attachment'] if 'attachment' in item else 0])
            idx += 1
    else:
        result.emit(None)
    ...
    if conn is not None:
        with conn:
            ...
            if row is not None:
                ...
                subject_end = row[8]
                cur.execute(
                    '''SELECT * FROM emails WHERE ID BETWEEN ? and ?''', (start, end))
                all_emails = cur.fetchall()
                cur.execute(
                    '''SELECT * FROM emailnotfound WHERE ID BETWEEN ? and ?''', (email_start, email_end))
                emails_not_found = cur.fetchall()
                cur.execute(
                    '''SELECT * FROM subjectnotfound WHERE ID BETWEEN ? and ?''', (subject_start, subject_end))
